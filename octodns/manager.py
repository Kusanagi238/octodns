...
class Manager(object):
    ...
    def __init__(
        ...
        self.include_meta = self._config_include_meta(manager_config, include_meta)
        ...
        if self.auto_arpa:
            ...
            self.global_post_processors = [auto_arpa.name] + self.global_post_processors
    ...
    def _config_executor(self, manager_config, max_workers=None):
        max_workers = (
            manager_config.get("max_workers", 1) if max_workers is None else max_workers
        )
    ...
    def _config_plan_outputs(self, plan_outputs_config):
        ...
        for plan_output_name, plan_output_config in plan_outputs_config.items():
            ...
            try:
                plan_outputs[plan_output_name] = _class(plan_output_name, **kwargs)
    ...
    def _try_version(self, module_name, module=None, version=None):
        ...
        return getattr(module, "__version__", getattr(module, "__VERSION__", None))
    ...
    def _get_named_class(self, _type, _class, context):
        try:
            ...
            self.log.exception("_get_{}_class: Unable to import module %s", _class)
            raise ManagerException(f"Unknown {_type} class: {_class}, {context}")
    ...
    def _populate_and_plan(
        ...
        for target in targets:
            ...
            for processor in processors:
                plan = processor.process_plan(plan, sources=sources, target=target)
            if plan:
    ...
    def _get_sources(self, decoded_zone_name, config, eligible_sources):
        try:
            ...
            raise ManagerException(
                f"Requested zone: {decoded_zone_name} is missing sources"
            )

        if eligible_sources and not [s for s in sources if s in eligible_sources]:
            return None
        ...
        try:
            ...
            raise ManagerException(
                f"Requested zone: {decoded_zone_name}, unknown " f"source: {source}"
            )
    ...
    def _preprocess_zones(self, zones, eligible_sources=None, sources=None):
        ...
        for name, config in list(zones.items()):
            ...
            # find its sources
            found_sources = sources or self._get_sources(name, config, eligible_sources)
    ...
    def sync(
        ...
        for zone_name, config in zones.items():
            ...
            sources = self._get_sources(decoded_zone_name, config, eligible_sources)
            ...
            try:
                collected = []
                for processor in (
                    self.global_processors + processors + self.global_post_processors
                ):
                    collected.append(self.processors[processor])
                processors = collected
            except KeyError:
                raise ManagerException(
                    f"Zone {decoded_zone_name}, unknown " f"processor: {processor}"
                )
            ...
            if self.auto_arpa and zone_name.endswith("arpa."):
                ...
                futures.append(self._executor.submit(self._populate_and_plan, **kwargs))
        ...
        if delayed_arpa:
            ...
            self.log.info("sync: processing %d delayed arpa zones", len(delayed_arpa))
        ...
        for target, plan in plans:
            ...
            if zones[zone_name].get("always-dry-run", False):
                self.log.info("sync: zone=%s skipping always-dry-run", zone_name)
                continue
    ...
    def dump(
        ...
        if output_provider:
            self.log.info("dump: using specified output_provider=%s", output_provider)
            try:
    ...
    def validate_configs(self, lenient=False):
        ...
        for zone_name, config in zones.items():
            ...
            try:
                ...
                raise ManagerException(f"Zone {decoded_zone_name} is missing sources")
